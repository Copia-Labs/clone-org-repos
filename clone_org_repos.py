...
def write_scheduler_scripts(settings, config_path, app_runner, out_dir):
    ...
    with open(bat_path, "w", encoding="utf-8") as fh:
        fh.write(header_bat)
        fh.write(
            "REM Redirect stdout+stderr to a log. A windowed/no-console run\r\n"
            "REM (Task Scheduler) otherwise has a null stdout; older builds\r\n"
            "REM crash writing to it, and this also captures any traceback.\r\n"
        )
        fh.write(
            f'{app_runner["bat"]} {quoted} > "%~dp0run_clone_console.log" 2>&1\r\n'
        )
    ...
    with open(ps1_path, "w", encoding="utf-8") as fh:
        fh.write(header_ps1)
        fh.write(
            "# Redirect all streams to a log. A windowed/no-console run\r\n"
            "# (Task Scheduler) otherwise has a null stdout; older builds\r\n"
            "# crash writing to it, and this also captures any traceback.\r\n"
        )
        fh.write(
            f'& {app_runner["ps1"]} {quoted} '
            f'*> "$PSScriptRoot\\run_clone_console.log"\r\n'
        )
